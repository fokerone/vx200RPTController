...
class VX200Controller {
    ...
    async initializeAuxiliaryServices() {
        ...
        // Auto-Updater (después del OLED para poder mostrar mensajes)
        try {
            this.modules.autoUpdater = new AutoUpdater(this.display);
            this.logger.info('Auto-Updater inicializado');
        } catch (error) {
            this.logger.error('Error inicializando Auto-Updater:', error.message);
            this.initializationErrors.push('AutoUpdater');
        }
    }
    ...
    stop() {
        ...
        if (this.modules.autoUpdater) {
            this.modules.autoUpdater.stop();
        }
    ...
    async startMonitoringServices() {
        ...
        setTimeout(async () => {
            try {
                ...
                if (this.modules.autoUpdater) {
                    try {
                        this.modules.autoUpdater.start();
                        this.logger.info('Auto-Updater activo');
                    } catch (error) {
                        this.logger.warn('Error iniciando Auto-Updater:', error.message);
                    }
                }
